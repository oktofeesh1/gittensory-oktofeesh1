...
export function buildPreStartCheck(
  ...
  if (typeof target.issueNumber === "number") {
    ...
    // fixed number of open issues so title matching stays cheap on repos with very large queues, and
    // compute the Jaccard score arithmetically (count shared terms; derive the union size from the
    // two set sizes) to avoid allocating an intermediate union set per issue.
    if (wanted.size > 0) {
      for (const issue of openIssues.slice(0, TITLE_MATCH_MAX_ISSUES)) {
        const have = new Set(tokenize(issue.title));
        let shared = 0;
        for (const term of wanted) if (have.has(term)) shared += 1;
        // `wanted` is non-empty here, so the union size is always > 0.
